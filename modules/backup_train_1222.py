...
class NERTrainer(BaseTrainer):
    ...
    def multiModal_before_train(self):
        ...
        parameters = [
            {'params': [p for name, p in self.model.named_parameters() if not any(nd in name for nd in no_decay) and ('bert' in name or 'Cross_atten' in name)], 
             'weight_decay': 1e-2,
             'lr': self.args.bert_lr},
            {'params': [p for name, p in self.model.named_parameters() if any(nd in name for nd in no_decay) and ('bert' in name or 'Cross_atten' in name)], 
             'weight_decay': 0.0,
             'lr': self.args.bert_lr},
            {'params': [p for name, p in self.model.named_parameters() if not any(nd in name for nd in no_decay) and 'crf' in name], 
             'weight_decay': 1e-2,
             'lr': self.args.crf_lr},
            {'params': [p for name, p in self.model.named_parameters() if any(nd in name for nd in no_decay) and 'crf' in name], 
             'weight_decay': 0.0,
             'lr': self.args.crf_lr},
            {'params': [p for name, p in self.model.named_parameters() if not any(nd in name for nd in no_decay) and ('classifier' in name or 'image_model' in name)], 
             'weight_decay': 1e-2,
             'lr': self.args.other_lr},
            {'params': [p for name, p in self.model.named_parameters() if any(nd in name for nd in no_decay) and ('classifier' in name or 'image_model' in name)], 
             'weight_decay': 0.0,
             'lr': self.args.other_lr},
        ]
