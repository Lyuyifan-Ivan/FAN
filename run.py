...
from models.FANet_model import TI2FANetModel
...
MODEL_CLASSES = {
    'twitter15': TI2FANetModel,
    'twitter17': TI2FANetModel
}
...
def main():
    ...
    parser.add_argument('--final_dropout', default=0.5, type=float)
    ...
    train_dataset = dataset_class(processor, transform, img_path, args.max_seq, mode='train')
    ...
    model = TI2FANetModel(label_list, args)

    trainer = Trainer(train_data=train_dataloader, dev_data=dev_dataloader, test_data=test_dataloader,
                      model=model, label_map=label_mapping, args=args, logger=logger, writer=writer)
